...
print("Starting clean and merge pipeline...")
...
for file in files:
    ...
    df["address_norm"] = normalize_address(df["address"]) if "address" in df.columns else None
    df["lat_round6"] = df["latitude"].round(6) if "latitude" in df.columns else None
    df["lon_round6"] = df["longitude"].round(6) if "longitude" in df.columns else None
...
panel.to_csv("Data/Processed/energy_benchmarking_panel.csv", index=False)

print(f"Saved Data/Processed/energy_benchmarking_panel.csv with {panel.shape[0]} rows and {panel.shape[1]} columns.")
print("Clean and merge pipeline finished.")